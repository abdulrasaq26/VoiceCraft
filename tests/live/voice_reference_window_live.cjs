// The opening of a voice reference survives, and 25s fits.
//
...
//   2.0 - 30.0s  loud dense speech-like noise  amplitude 0.90
//
...
(async () => {
  ...
  const out = await page.evaluate(async () => {
    ...
    // 1. Thirty seconds — longer than the ceiling, so it must be cut, and the
    //    cut must be at the end.
    const long = await VC.prepare(build(30, 2));
    results.long = {
    ...
    // 2. Twenty seconds — inside the ceiling, so nothing should be removed at
    //    all. This is the case in the screenshot: a 20.3s file became 14.0s.
    const short = await VC.prepare(build(20, 2));
    results.short = {
  ...
  check('the ceiling is 25 seconds', out.MAX_SEC === 25, out.MAX_SEC);
  ...
  check('a recording past the ceiling is cut down to it',
        L.seconds > 23.5 && L.seconds <= 25.05, L);
