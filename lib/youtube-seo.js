...
function parseSeo(rawText, project) {
  return normalize(text.extractJson(rawText), project);
}

async function buildSeo(project, channel) {
  if (MOCK) return normalize(mockSeo(project, channel), project);
  const p = seoPrompt(project, channel);
  const json = await text.generate(p.system, p.user, { json: true, temperature: 0.85 });
...
module.exports = { buildSeo, seoPrompt, parseSeo, configured };
