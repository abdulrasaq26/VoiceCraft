...
function parseBible(rawText) {
  return normalizeBible(text.extractJson(rawText));
}

async function buildBible(context) {
  if (MOCK) return mockBible(context);
  const p = biblePrompt(context);
  const bible = await text.generate(p.system, p.user, { json: true, temperature: 0.6 });
...
function parseScenes(rawText, cues) {
  const result = text.extractJson(rawText);
...
async function buildScenes({ bible, cues, style, instructions, priorSummaries }) {
  ...
  const p = scenesPrompt({ bible, cues, style, instructions, priorSummaries });
  const result = await text.generate(p.system, p.user, { json: true, temperature: 0.85 });
  const scenes = Array.isArray(result?.scenes) ? result.scenes : [];
  return { scenes: scenes.map((s, i) => normalizeScene(s, cues[i], cues)) };
}
