...
// rateMax: Chirp voices document a narrower speaking-rate range (0.25–2.0)
// than the general API limit (0.25–4.0).
function capabilitiesFor(voiceId) {
  const family = familyOf(voiceId);
  if (['Chirp3-HD', 'Chirp-HD', 'Chirp'].includes(family)) {
    return { rate: true, rateMax: 2.0, pitch: false, ssml: false };
  }
  if (['Journey', 'Gemini', 'Other'].includes(family)) {
    return { rate: false, rateMax: 4.0, pitch: false, ssml: false };
  }
  if (family === 'Studio') {
    return { rate: true, rateMax: 4.0, pitch: false, ssml: true };
  }
  return { rate: true, rateMax: 4.0, pitch: true, ssml: true };
}
