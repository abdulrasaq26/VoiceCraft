...
// Per-operation timeouts. Synthesis of HD voices (Chirp3-HD, Studio) with
// longer text can legitimately take much longer than listing voices or
// generating a short preview, so they don't share one flat budget.
// Overridable via env for constrained hosts.
const SYNTH_TIMEOUT_MS = Number(process.env.TTS_SYNTH_TIMEOUT_MS) || 90000;
const LIST_TIMEOUT_MS = Number(process.env.TTS_LIST_TIMEOUT_MS) || 20000;
const PREVIEW_TIMEOUT_MS = Number(process.env.TTS_PREVIEW_TIMEOUT_MS) || 30000;
...
async function authHeader() {
  ...
  // Guard the token fetch: without a timeout a misconfigured metadata
  // server on the host could hang the whole request indefinitely.
  const token = await withTimeout(
    auth.getAccessToken(),
    AUTH_TIMEOUT_MS,
    'Timed out obtaining Google credentials. Check the service account / ADC configuration.'
  );
...
async function googleFetch(base, pathname, { method = 'GET', body, timeoutMs = LIST_TIMEOUT_MS } = {}) {
...
async function listVoices() {
  ...
  const body = await googleFetch(V1, '/voices', { timeoutMs: LIST_TIMEOUT_MS });
  return body.voices || [];
}

// request: a SynthesizeSpeechRequest body. Returns the audio as a Buffer.
// beta: use v1beta1 (needed for prompt-capable voices).
// preview: use the shorter preview timeout (previews are short phrases).
async function synthesize(request, { beta = false, preview = false } = {}) {
  if (MOCK) return mockWav();
  const body = await googleFetch(beta ? V1BETA1 : V1, '/text:synthesize', {
    method: 'POST',
    body: request,
    timeoutMs: preview ? PREVIEW_TIMEOUT_MS : SYNTH_TIMEOUT_MS
  });
